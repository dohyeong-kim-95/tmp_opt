...
> 자가 점검과 전체 워크플로(데이터 생성 → 반응표면 → run)가 완주하는 것.**
...
## 3.6 파일 교환 게이트 — "데이터가 안 남는" 실행 금지

모듈 간 계약은 **파일을 주고받으며 다음 iteration 을 만드는 것**이다. run 이
...
3. 완주 후 산출물이 레퍼런스 reader(`read_x`/`read_y_raw`/`load_history`/
   `load_state`)로 그대로 읽히고 정합해야 한다.

"통과했다"의 증거는 콘솔 로그가 아니라 **디스크에 남은(또는 run 중 churn 한)
교환 파일**이다.
...
- **측정/판정 정의 변경**: convert_y_raw 의 측정 정의, 점수 파이프라인의
  수학, 반응표면의 커버리지 게이트를 바꾸는 것은 버그 수정이 아니라 문제
  재정의다 — 감독 승인 없이 금지
...
3. runner/calculator/make_dataset 을 만들지 않고 optimizer 한 파일에 전부
   욱여넣은 뒤 "통과"라고 보고 → 폐기됨 (§3.5)
