...
- 출력 y0 : 6개 스칼라 (y11, y12, y13, y21, y22, y23).
    * 최대화: y11, y12, y21, y22 / 최소화: y13, y23
    * j(뒤 숫자)가 같으면 스케일이 비슷하고, j가 다르면 스케일이 크게 다르다.
      값의 범위는 사전에 알 수 없다고 가정한다 (스케일러가 온라인으로 추정).
    * 관측 노이즈: 주효과(latent) 표준편차의 5% 크기 가우시안.
...
class BenchmarkBase:
    ...
    def __init__(self, noise_seed: int = 0, noise_level: float = 0.05) -> None:
        ...
        # 노이즈 크기 산정: latent 주효과 std를 몬테카를로로 추정해 두고,
        # 관측 시 sigma = noise_level × (latent std) × |scale| 를 사용한다.
    ...
    def evaluate(self, x: np.ndarray, noisy: bool = True) -> np.ndarray:
        """X 하나 또는 배치를 평가해 raw 목적값 y0을 반환한다.
        ...
        Returns:
            (6,) 또는 (n, 6) — 은닉 스케일이 적용된 raw 목적값
        """
        x = np.atleast_2d(np.asarray(x, dtype=np.int64))
        assert x.shape[1] == self.space.n_cols, f"X는 30컬럼이어야 함: {x.shape}"

        latent = self._latent(self.space.to_unit(x))  # (n, 6)
        if noisy:
            # 주효과(latent std)의 noise_level 배 크기 가우시안 노이즈
            sigma = self.noise_level * self._latent_std  # (6,)
            latent = latent + self._noise_rng.normal(0.0, 1.0, latent.shape) * sigma

        self.n_evaluations += int(x.shape[0])
        y = latent * self._scales + self._offsets  # 은닉 스케일/오프셋 적용
...
if __name__ == "__main__":
    # 간단한 자가 점검: 공간 크기, 스케일 상이함, 노이즈 수준 확인
    space = SearchSpace()
    ...
    for name, cls in BENCHMARKS.items():
        ...
        y = calc.evaluate(xs)
        y_clean = calc.evaluate(xs, noisy=False)
        print(f"\n[{name}] y0 예시 (raw, 행=샘플):")
        header = "  " + "  ".join(f"{n:>12s}" for n in OBJECTIVE_NAMES)
        print(header)
        for row in np.atleast_2d(y):
            print("  " + "  ".join(f"{v:12.4g}" for v in row))
        noise_ratio = np.abs(y - y_clean).mean(axis=0) / (
            np.abs(y_clean).mean(axis=0) + 1e-12
        )
        print("  noise/|signal| ≈", np.round(noise_ratio, 4))
